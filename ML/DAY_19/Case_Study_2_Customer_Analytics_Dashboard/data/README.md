# `data/` — Customer Analytics dashboard data

Two copies of the **same synthetic records**, in the two formats a BI workflow needs:

| File | Format | Used by |
|------|--------|---------|
| `customer_data.csv` | comma-separated | **Import into Power BI / Tableau** (or open in Excel) to rebuild the dashboard in those tools |
| `customer_data.js` | JavaScript (`window.CUSTOMER_DATA = [ ... ]`) | Loaded by `../dashboard.html` so the offline HTML dashboard has its data with no server |
