# `data/` — HR Workforce dashboard data

Two copies of the **same synthetic records**, in the two formats a BI workflow needs:

| File | Format | Used by |
|------|--------|---------|
| `hr_data.csv` | comma-separated | **Import into Power BI / Tableau** (or open in Excel) to rebuild the dashboard in those tools |
| `hr_data.js` | JavaScript (`window.HR_DATA = [ ... ]`) | Loaded by `../dashboard.html` so the offline HTML dashboard has its data with no server |
